...
def main():
    ...
    if not reports_dir.exists():
        print(f"Reports directory not found: {args.reports}")
        return
    ...
    # --- Output 1: research_status_summary.json ---
    json_output = reports_dir / "research_status_summary.json"
    json_data = {
        "generated_by": "research_report_miner.py",
        "record_count": len(results),
        "records": [
            {
                "source_file": r.source_file,
                "project": _infer_project(r.source_file),
                "study_name": r.study,
                "symbol": r.symbol,
                "source_venue": r.source_venue,
                "target_venue": r.target_venue,
                "signal_type": r.signal_type,
                "horizon_ms": r.horizon_ms,
                "events_count": r.total_signals,
                "valid_events": r.valid_events,
                "gross_bps": r.gross_bps,
                "fee_bps": r.fee_bps,
                "slippage_bps": r.slippage_bps,
                "latency_buffer_bps": r.latency_bps,
                "quote_mismatch_bps": None,
                "net_bps": r.net_bps,
                "win_rate": r.win_rate,
                "random_baseline_bps": r.baseline_net_bps,
                "beats_baseline": r.beats_baseline,
                "verdict": r.verdict,
                "rejection_reason": "; ".join(r.rejection_reasons[:5]),
                "warnings": [],
            }
            for r in results
        ],
    ...
    # --- Output 2: research_status_summary.csv ---
    csv_output = reports_dir / "research_status_summary.csv"
    with open(csv_output, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "source_file", "project", "study_name", "symbol",
            "source_venue", "target_venue", "signal_type", "horizon_ms",
        ...
        for r in sorted(results, key=lambda x: (x.verdict != "CANDIDATE", x.study)):
            writer.writerow([
                r.source_file, _infer_project(r.source_file), r.study, r.symbol,
                r.source_venue, r.target_venue, r.signal_type, r.horizon_ms,
                r.total_signals, r.gross_bps, r.fee_bps, r.slippage_bps,
                r.latency_bps, None,
                r.net_bps, r.win_rate, r.baseline_net_bps, r.beats_baseline,
                r.verdict, "; ".join(r.rejection_reasons[:5]), "",
            ])
    print(f"Written: {csv_output}")

    # --- Output 3: research_status_summary.md ---
    # Combined status + bps-gate in one readable file
    md_lines: list[str] = [
    ...
    verdict_order = {"CANDIDATE": 0, "UNKNOWN": 1, "NEEDS_MORE_DATA": 2, "REJECTED": 3}
    for r in sorted(results, key=lambda x: (verdict_order.get(x.verdict, 99), x.study, x.signal_type)):
        venues = ""
        if r.source_venue and r.target_venue:
            venues = f"{r.source_venue}->{r.target_venue}"
        elif r.source_venue:
    ...
    for r in results:
        verdicts[r.verdict] = verdicts.get(r.verdict, 0) + 1
    for v in ["CANDIDATE", "UNKNOWN", "NEEDS_MORE_DATA", "REJECTED"]:
        count = verdicts.get(v, 0)
        if count > 0:
            md_lines.append(f"- **{v}**: {count} study groups")

    md_lines.append("")
    md_lines.append("## BPS Gate Table")
    md_lines.append("")
    md_lines.append("| Study | Signal | Symbol | Venues | Gross | Fee | Slip | Net | Baseline | Win% | Events | Cand? |")
    md_lines.append("|-------|--------|--------|--------|-------|-----|------|-----|----------|------|--------|-------|")
    for r in sorted(results, key=lambda x: x.net_bps if x.net_bps is not None else -999, reverse=True):
        venues = f"{r.source_venue}->{r.target_venue}" if r.source_venue and r.target_venue else ""
        gross = f"{r.gross_bps:.2f}" if r.gross_bps is not None else "-"
    ...
    md_lines.append("")
    md_lines.append("## Locked Gates")
    md_lines.append("")
    md_lines.append("1. **Kraken BTC/USD spot OHLCV indicators** (5m, 1h, Donchian, EMA, ATR). ~80 bps round-trip taker fees. Rejected V1-V4.")
    md_lines.append("2. **Same-asset cross-venue tick lead-lag** (CB<->KRK BTC/ETH). HFT-dominated. Rejected.")
    md_lines.append("3. **Naive top-of-book L2 maker** on BTC/ETH at current fee tier. Rejected.")
    md_lines.append("4. **Direct cash-and-carry** under tested Kraken spot + Binance/Bybit perp cost model. Rejected.")
    md_lines.append("")

    md_output = reports_dir / "research_status_summary.md"
