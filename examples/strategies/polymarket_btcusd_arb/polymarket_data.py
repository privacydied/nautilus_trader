...
def _extract_strike(info):
    text=' '.join(str(info.get(k,'')) for k in ('question','description','title'))
    m=re.search(r'\$?([0-9]{2,3}(?:,[0-9]{3})+(?:\.\d+)?)',text) or re.search(r'\$?([0-9]{4,6}(?:\.\d+)?)',text)
    return float(m.group(1).replace(',','')) if m else 100000.0

def _extract_updown_expiry_ns(info,fallback_expiry_ns):
    slug=str(info.get('market_slug') or info.get('slug') or '')
    m=re.search(r'updown-(\d+)m-(\d{10})',slug)
    if m:
        return (int(m.group(2))+int(m.group(1))*60)*1_000_000_000
    return int(fallback_expiry_ns)

def _extract_updown_tokens(tokens):
    yes=no=None
    for tok in tokens:
        if not isinstance(tok,dict): continue
        outcome=str(tok.get('outcome','')).upper(); token=str(tok.get('token_id') or tok.get('asset_id') or '')
        if outcome in {'YES','UP'}: yes=token
        if outcome in {'NO','DOWN'}: no=token
    return yes,no,bool(no)
...
async def fetch_polymarket_trades(market_slug,*,sanitize_info=True,start=None,end=None):
    ...
    info=getattr(loader.instrument,'info',{}) or {}; tokens=info.get('tokens') or []; yes,no,has_no=_extract_updown_tokens(tokens)
    expiry_ns=_extract_updown_expiry_ns(info,int(loader.instrument.expiration_ns))
    meta=PolymarketContractMetadata(market_slug,_extract_strike(info),expiry_ns,str(info.get('condition_id') or info.get('conditionId') or ''),yes,no,has_no_token=has_no,sanitized_info=True,resolution_metadata=getattr(loader,'resolution_metadata',{}) or {})
