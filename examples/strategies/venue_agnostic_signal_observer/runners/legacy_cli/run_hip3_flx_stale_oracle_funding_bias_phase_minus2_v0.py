...
_PKG_ROOT = str(Path(__file__).resolve().parents[6])
...
def _list_s3_replica_cmds(date_str: str, max_keys: int = 100) -> list[str]:
    """List replica_cmds keys for a given date via aws s3 api.

    S3 layout: replica_cmds/<ISO_timestamp>/<YYYYMMDD>/<block>.lz4
    Converts YYYY-MM-DD to YYYYMMDD and performs multi-level listing.

    Searches across ALL relevant timestamp prefixes (most recent first)
    until max_keys are found or no more prefixes remain. Does NOT stop
    after the first prefix — a date may live under an older timestamp
    prefix if the most recent one does not cover it.

    Returns list of full relative keys.
    """
    import subprocess
    import json
    from datetime import datetime, timedelta

    # Convert "2026-05-28" -> "20260528"
    target_date_yyyymmdd = date_str.replace("-", "")
    try:
        target_dt = datetime.strptime(target_date_yyyymmdd, "%Y%m%d")
        # S3 timestamp prefixes can be up to ~14 days before the data date
        ts_early_skip_prefix = (target_dt - timedelta(days=14)).strftime("%Y-%m-%dT")
    ...
    try:
        ...
        l1_result = subprocess.run(
            ["aws", "s3api", "list-objects-v2",
             f"--bucket={REPLICA_CMDS_BUCKET}",
             f"--prefix={REPLICA_CMDS_PREFIX}/",
             "--delimiter=/",
             "--max-keys=500",
        ...
        if l1_result.returncode != 0:
            print(f"  S3 level-1 listing failed: {l1_result.stderr.strip()}", flush=True)
            return []
        l1_data = json.loads(l1_result.stdout)
        l1_prefixes = l1_data.get("CommonPrefixes", [])
        if not l1_prefixes:
            print(f"  No timestamp prefixes found under replica_cmds/", flush=True)
            return []

        # Sort prefixes descending so we check the most recent first
        sorted_prefixes = sorted(l1_prefixes, key=lambda p: p["Prefix"], reverse=True)

        # Level 2: for each timestamp prefix (most recent first), list date prefixes
        all_keys = []
        for ts_prefix_entry in sorted_prefixes:
            ts_prefix = ts_prefix_entry["Prefix"]
            # Skip timestamps clearly before the target window
            ts_date_part = ts_prefix.rstrip("/").split("/")[-1].split("T")[0]
            if ts_early_skip_prefix and ts_date_part < ts_early_skip_prefix:
                continue
            ...
            if l2_result.returncode != 0:
                continue
            l2_data = json.loads(l2_result.stdout)
            l2_prefixes = l2_data.get("CommonPrefixes", [])
            for dp_prefix_entry in l2_prefixes:
                dp_prefix = dp_prefix_entry["Prefix"]
                date_segment = dp_prefix.rstrip("/").split("/")[-1]
                if date_segment != target_date_yyyymmdd:
                    continue
                # Level 3: list block files under matching date prefix
                ...
                if l3_result.returncode != 0:
                    continue
                l3_data = json.loads(l3_result.stdout)
                for obj in l3_data.get("Contents", []):
                    key = obj["Key"]
                    if key.endswith(".lz4"):
                        all_keys.append(key)
                break  # Found the date in this timestamp prefix
            if len(all_keys) >= max_keys:
                break
            # Do NOT break after first match — older timestamp prefixes
            # may also contain the same date, and we need to find at least
            # some files. Only stop if we have enough keys.
...
def _estimate_s3_bytes(keys: list[str]) -> tuple[int, int]:
    """Estimate total bytes for a list of keys via list-objects-v2 metadata.

    Uses a single list-objects-v2 call per key's parent prefix to read
    ContentLength directly from listing metadata. No per-key head-object.

    Returns (total_bytes, list_object_count).
    """
    import json
    import subprocess
    ...
    # Group keys by their parent prefix to minimize list calls
    prefix_keys: dict[str, list[str]] = {}
    for key in keys:
        # Strip the timestamp prefix to get the date-level prefix
        # e.g. "replica_cmds/2026-05-07T09:45:57Z/20260528/block.lz4"
        parts = key.split("/", 3)
        if len(parts) >= 3:
            # date prefix = parts[0]/parts[1]/parts[2]
            prefix = "/".join(parts[:3]) + "/"
        else:
            prefix = ""
    ...
    for prefix, prefix_key_list in prefix_keys.items():
        try:
            result = subprocess.run(
                ["aws", "s3api", "list-objects-v2",
                 f"--bucket={REPLICA_CMDS_BUCKET}",
            ...
            for obj in data.get("Contents", []):
                ...
                if obj_key in prefix_key_list:
                    total_bytes += obj_size
                    listed_count += 1
        except Exception:
            pass

    return total_bytes, listed_count
...
# ---------------------------------------------------------------------------
# Frequency scan runner
# ---------------------------------------------------------------------------

def run_frequency_scan(args: argparse.Namespace) -> dict:
    """Run a cheap FLX target-symbol frequency scan.

    Decodes replica_cmds, counts oracle updates for target DEX/symbols,
    discards everything else. Does NOT run alignment, residuals, or gates.
    """
    run_id = _run_id()
    out_root = Path(args.out_root) / run_id
    out_root.mkdir(parents=True, exist_ok=True)

    print(f"Phase -2 FLX oracle frequency scan", flush=True)
    ...
    # Target update counts: {flx:TSLA: count, flx:NVDA: count, ...}
    flx_target_update_counts: dict[str, int] = {
        f"{target_dex}:{sym}": 0 for sym in symbols
    }
    # Reference update counts: {cash:TSLA: count, ...}
    reference_update_counts: dict[str, int] = {}
    # All oracle update counts for context: {dex:sym: count}
    all_oracle_update_counts: dict[str, int] = defaultdict(int)
    # Counts by date: {date_str: {dex:sym: count}}
    ...
    listed_object_count_by_date: dict[str, int] = {}
    creation_timestamp_prefixes_checked: set[str] = set()
    selected_objects_detail: list[dict] = []
    selected_object_count_by_date: dict[str, int] = {}
    ...
    if args.local_replica_cmds_dir:
        ...
        for date_str in sample_dates:
            ...
            if date_dir.exists():
                for fp in sorted(date_dir.glob("*.lz4"))[:args.max_files]:
                    plan_files.append(f"local:{date_str}/{fp.name}")
            else:
                # Also check root of local dir for flat layout
                for fp in sorted(local_dir_path.glob("*.lz4"))[:args.max_files]:
                    plan_files.append(f"local:{date_str}/{fp.name}")
    elif args.allow_s3_archive_read:
        for date_str in sample_dates:
            keys = _list_s3_replica_cmds(date_str, max_keys=args.max_files)
            listed_object_count_by_date[date_str] = len(keys)
    ...
    if args.allow_s3_archive_read and plan_files:
        total_estimated, listed_count = _estimate_s3_bytes(plan_files)
        budget_state["total_estimated_bytes"] = total_estimated
        budget_state["listed_object_count"] = listed_count
        print(f"  Estimated download size: {total_estimated / 1e6:.1f} MB ({listed_count} objects listed, 0 head-object calls)", flush=True)
    ...
    # Decode files and count (process ALL plan files, not just max_files total)
    file_count = 0
    total_oracle_updates = 0
    decode_inventory = {
        "files_processed": [],
        "decode_failures": 0,
    ...
    def _count_payloads(payloads, file_key, date_str):
        """Count oracle payloads into the appropriate buckets."""
        nonlocal total_oracle_updates
        for payload in payloads:
            px = payload.get("px")
            if px is None or _safe_float(px) is None:
                continue
            ...
            key = f"{dex}:{sym}"
            all_oracle_update_counts[key] += 1
            decode_inventory["updates_by_dex_symbol"][key] += 1
            total_oracle_updates += 1

            # Count target
            if dex == target_dex and sym.upper() in symbols:
                target_key = f"{dex}:{sym.upper()}"
                flx_target_update_counts[target_key] = flx_target_update_counts.get(target_key, 0) + 1

            # Count reference
            if dex in reference_dexes:
                ref_key = f"{dex}:{sym.upper()}"
                reference_update_counts[ref_key] = reference_update_counts.get(ref_key, 0) + 1

            # Count by date
            if date_str:
                counts_by_date.setdefault(date_str, defaultdict(int))
                counts_by_date[date_str][key] += 1

    for file_key in plan_files:
        if file_count >= len(plan_files):
            break
        ...
        # Track selected object detail
        if file_date:
            # Estimate size from S3 listing
            obj_size = 0
            if args.allow_s3_archive_read:
                parts = file_key.split("/", 3)
                if len(parts) >= 3:
                    ts_prefix = parts[1].split("T")[0] if "T" in parts[1] else parts[1]
                    creation_timestamp_prefixes_checked.add(f"replica_cmds/{parts[1]}/")
            selected_objects_detail.append({
                "date": file_date,
                "s3_key": file_key,
                "compressed_bytes": obj_size,
            })
            selected_object_count_by_date[file_date] = selected_object_count_by_date.get(file_date, 0) + 1

        if file_key.startswith("local:"):
            # Key format: local:date_str/filename.lz4 or local:filename.lz4
            rest = file_key[6:]  # strip "local:"
            local_path = Path(args.local_replica_cmds_dir) / rest
            if not local_path.exists():
                # Try without date prefix for flat layout
                parts = rest.split("/", 1)
                if len(parts) == 2:
                    local_path = Path(args.local_replica_cmds_dir) / parts[1]
            if not local_path.exists():
            ...
            if not updates:
                decode_inventory["decode_failures"] += 1
                continue
            # We need the raw data to re-count payloads; download_and_decode_file
            # already downloaded it. Read it back.
            local_path = local_dir / Path(file_key).name
            with open(local_path, "rb") as f:
                data = f.read()
        ...
        for rec_idx, obj, raw_line in _decode_lz4_json_records(data):
            if obj is None:
                ...
                continue
            payloads = _extract_oracle_payloads_from_record(obj)
            _count_payloads(payloads, file_key, file_date)
            file_oracle_count += len(payloads)

        decode_inventory["oracle_updates_extracted"] += file_oracle_count
        file_count += 1
    ...
    counts_by_date_dict = {k: dict(v) for k, v in counts_by_date.items()}

    # Compute frequency scan status
    status, projection = _compute_frequency_scan_status(
        flx_target_update_counts=flx_target_update_counts,
        reference_update_counts=reference_update_counts,
        decoded_file_count=budget_state["files_decoded"],
        total_oracle_updates=total_oracle_updates,
        min_target_updates=args.min_target_updates,
        selected_compressed_bytes=budget_state["bytes_downloaded"],
    )

    # Determine full_bias_run_warranted
    full_bias_run_warranted = False
    if status == "HIP3_FLX_ORACLE_FREQUENCY_SCAN_TARGET_FOUND":
        for key, count in flx_target_update_counts.items():
            ...
            if projected >= args.min_target_updates:
                full_bias_run_warranted = True
                break

    # Write artifacts
    _write_json_artifact(out_root / "run_manifest.json", {
        ...
        "mode": "frequency_scan_only",
        "final_status": status,
        "phase0_review_allowed": False,
        "paper_or_live_allowed": False,
        "registry_mutation_allowed": False,
        "actual_funding_measured": False,
        "full_bias_run_warranted": full_bias_run_warranted,
    ...
    _write_json_artifact(out_root / "input_plan.json", {
        "symbols": symbols,
        "target_dex": target_dex,
        "reference_dexes": reference_dexes,
        "sample_dates": sample_dates,
    ...
    # flx_target_frequency_counts.csv
    with open(out_root / "flx_target_frequency_counts.csv", "w") as f:
        f.write("dex,symbol,count\n")
        for key in sorted(flx_target_update_counts.keys()):
            f.write(f"{key},{flx_target_update_counts[key]}\n")

    # oracle_update_counts_by_dex_symbol.csv
    with open(out_root / "oracle_update_counts_by_dex_symbol.csv", "w") as f:
        f.write("dex,symbol,count\n")
        for key, count in sorted(all_oracle_update_counts.items()):
            parts = key.split(":", 1)
            f.write(f"{parts[0]},{parts[1]},{count}\n")

    # frequency_scan_summary.json
    # Build all_flx_update_counts: only keys where dex == target_dex
    all_flx_update_counts: dict[str, int] = {}
    for key, count in all_oracle_update_counts.items():
        if ":" in key and key.split(":", 1)[0].lower() == target_dex:
            all_flx_update_counts[key] = count

    # Build flx_target_counts_by_date: per-date counts for target symbols only
    flx_target_counts_by_date: dict[str, dict[str, int]] = {}
    for date_str, date_counts in counts_by_date_dict.items():
        day_targets: dict[str, int] = {}
        for sym in symbols:
            target_key = f"{target_dex}:{sym}"
            day_targets[target_key] = date_counts.get(target_key, 0)
        flx_target_counts_by_date[date_str] = day_targets
    ...
    freq_summary = {
        ...
        "reference_dexes": reference_dexes,
        "selected_object_count": listed_count,
        "selected_compressed_bytes": budget_state["bytes_downloaded"],
        "decoded_file_count": budget_state["files_decoded"],
        "decoded_oracle_update_count": total_oracle_updates,
        "all_flx_update_counts": all_flx_update_counts,
        "flx_target_update_counts": flx_target_update_counts,
        "flx_target_counts_by_date": flx_target_counts_by_date,
        "reference_update_counts": reference_update_counts,
        "counts_by_date": counts_by_date_dict,
        "updates_per_compressed_gb": projection.get("updates_per_compressed_gb", {}),
        "projected_updates_for_4_date_8_file_run": projection.get("projected_updates_for_4_date_8_file_run", {}),
        "projected_files_needed_for_30_target_updates": projection.get("projected_files_needed_for_30_target_updates", {}),
        "projected_bytes_needed_for_30_target_updates": projection.get("projected_bytes_needed_for_30_target_updates", {}),
        "frequency_scan_status": status,
        "full_bias_run_warranted": full_bias_run_warranted,
        "phase0_review_allowed": False,
        "paper_or_live_allowed": False,
        "registry_mutation_allowed": False,
        ...
        "listing_metadata": {
            ...
            "listed_object_count_by_date": dict(listed_object_count_by_date),
            "selected_object_count_by_date": dict(selected_object_count_by_date),
            "creation_timestamp_prefixes_checked": sorted(creation_timestamp_prefixes_checked),
            "selected_objects_detail": selected_objects_detail,
        },
    }
    _write_json_artifact(out_root / "frequency_scan_summary.json", freq_summary, pretty=True)

    # frequency_scan_summary.md
    md_lines = [
        f"# Phase -2 FLX Oracle Frequency Scan",
        f"",
        f"**Run ID:** {run_id}",
        f"**Status:** {status}",
        ...
        f"- Phase 0 review NOT authorized by frequency scan alone.",
        f"",
        "## Data",
        ...
        f"- Oracle updates extracted: {total_oracle_updates}",
        f"- Bytes downloaded: {budget_state['bytes_downloaded'] / 1e6:.1f} MB",
        f"",
        "## FLX Target Update Counts",
        "",
    ]
    for key, count in sorted(flx_target_update_counts.items()):
    ...
    for key, count in sorted(reference_update_counts.items()):
        md_lines.append(f"- `{key}`: {count}")
    ...
    md_lines.extend([
        ...
        "## Projection",
        "",
        f"- Full bias run warranted: {'Yes' if full_bias_run_warranted else 'No'}",
        f"- Phase 0 review allowed: No",
        "",
        "## Listing Metadata",
        "",
        f"- Dates queried: {', '.join(sample_dates)}",
    ])
    for attempt in listing_attempts:
        md_lines.append(f"- `{attempt['date']}`: {attempt['objects_listed']} objects listed")
    if selected_objects_detail:
        md_lines.append("")
        md_lines.append("### Selected Objects")
        md_lines.append("")
        for obj in selected_objects_detail:
            md_lines.append(f"- `{obj.get('s3_key', 'unknown')}` ({obj.get('date', 'unknown')})")
    ...
    with open(out_root / "frequency_scan_summary.md", "w") as f:
        f.write("\n".join(md_lines))

    # summary.json and summary.md (alias for compatibility)
    _write_json_artifact(out_root / "summary.json", freq_summary, pretty=True)
    with open(out_root / "summary.md", "w") as f:
        f.write("\n".join(md_lines))

    print()
    print(f"Status: {status}", flush=True)
    print(f"Phase 0 review allowed: False", flush=True)
    print(f"Full bias run warranted: {full_bias_run_warranted}", flush=True)
    print(f"Output directory: {out_root}", flush=True)
    print(f"Summary: {out_root / 'frequency_scan_summary.md'}", flush=True)
        ...
        for date_str in sample_dates:
            ...
            if date_dir.exists():
                for f in sorted(date_dir.glob("*.lz4"))[:args.max_files]:
                    plan_files.append(f"local:{date_str}/{f.name}")
            else:
                # Also check root of local dir for flat layout
                for f in sorted(local_dir_path.glob("*.lz4"))[:args.max_files]:
                    plan_files.append(f"local:{date_str}/{f.name}")
    elif args.allow_s3_archive_read:
        for date_str in sample_dates:
            keys = _list_s3_replica_cmds(date_str, max_keys=args.max_files)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--frequency-scan-only", action="store_true",
        help="Run only the cheap FLX target-symbol frequency scan (no alignment/gates)",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print(f"Frequency scan only: {args.frequency_scan_only}", flush=True)
        print()
        print("DRY_RUN_READY", flush=True)
        return

    if args.frequency_scan_only:
        summary = run_frequency_scan(args)
        # Validate frequency scan status
        status = summary.get("frequency_scan_status", "")
        if status not in ALLOWED_STATUSES:
            print(f"WARNING: unexpected status '{status}'", flush=True)
        if status in FORBIDDEN_STATUSES:
            print(f"ERROR: forbidden status '{status}'", flush=True)
        return
