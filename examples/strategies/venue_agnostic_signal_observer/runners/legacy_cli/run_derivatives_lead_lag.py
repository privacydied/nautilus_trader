...
from examples.strategies.venue_agnostic_signal_observer.derivatives_lead_lag import DerivativesImpulseGenerator, impulse_to_tick_signal
    ...
    for (sig_type, lb_ms), group_impulses in sorted(groups.items()):
        ...
        stats = {
            **group_key,
            ...
            "mean_net_return_bps": round(mean_net, 4) if mean_net is not None else None,
            "median_net_return_bps": round(median_net, 4) if median_net is not None else None,
            "win_rate": round(win_rate, 4) if win_rate is not None else None,
            "std_net_return_bps": round(std_net, 4) if std_net is not None else None,
        ...
        if not args.skip_baseline and source_ticks:
            from examples.strategies.venue_agnostic_signal_observer.tick_models import TradeTickLite as TT
