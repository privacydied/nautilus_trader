...
||| **Hyperliquid liquidation-cluster prepositioning Phase 0 v0** | Per-address isolated-margin liquidation-cluster map → continuation into/through clusters | Hyperliquid altcoin perpetuals | **NEEDS_MORE_DATA / PUBLIC_ARCHIVE_RECONSTRUCTION_BLOCKED** | Implementation only produced aggregate-OI proxy liquidation clusters; exact per-address isolated liquidation map was not reconstructable from available public archive inputs. Proxy diagnostic produced negative mark-return metrics (mean net50 ≈ -46.3 bps, median net50 ≈ -48.5 bps, win rate ≈ 26.1%), but these came from a fabricated aggregate-OI-imputed proxy map using assumed long/short splits and leverage defaults — not evidence against the true per-address isolated liquidation-cluster hypothesis. The proxy result is non-promotable and non-rejective. True mechanism untested — requires node fills archive access or public liquidation-price/margin-mode source. | `hyperliquid-liq-cluster-prepositioning-v0-proxy-blocked` |
