...
## Correction / Closure — Stale FLX Premise Falsified by Corrected Decoder
...
The original hypothesis (line 20) stated:

> `flx` oracle updates are sparse or stale relative to active builder DEX oracles (`cash`, `km`, `xyz`, `para`).

This premise was later found to be **invalidated** by a decoder bug.

### Root cause

The old `replica_cmds` extractor (on both `feat/hip3-flx-stale-oracle-funding-bias-phase-minus2-v0` and `feat/hip3-replica-cmds-setoracle-reconstruction-v0`) traversed only:

```
sa.action.setOracle.oraclePxs
```

But actual `replica_cmds` data wraps deployer oracle updates inside a multiSig envelope:

```
sa.action(type=multiSig).payload.action(type=perpDeploy).setOracle.oraclePxs
```

Since `action.type` is `"multiSig"`, not `"perpDeploy"`, the extractor silently skipped every flx oracle update. All `flx:*` keys were dropped. The prior `flx: 0` finding was a decoder artifact, not a market fact.

### Corrected evidence

A positive-control validation detected `flx:BTC` (131 occurrences) in `/tmp/test_block.lz4`, a known-control block where `flx:BTC` was previously observed manually. After the fix, the corrected 3-date reanalysis decoded 25,000 records and 95,152 oracle updates:
...
16 unique flx keys detected: BTC, COIN, COPPER, CRCL, GAS, GOLD, NVDA, OIL, PALLADIUM, PLATINUM, SILVER, TSLA, USA100, USA500, USDE, XMR.

`flx:TSLA` and `flx:NVDA` update actively at 16.18 updates/hour — comparable to `km` (~11.5) and `xyz` (~13.8). FLX is **not** sparse relative to references.

### Closure

The B-slow stale-oracle funding-distortion hypothesis is **closed upstream** because its core premise (flx oracle is sparse/stale) is unsupported by corrected oracle-frequency evidence. Alignment, bias, funding-clock proxy, and actual funding measurement were **not** run because the stale premise failed upstream.
...
### Forward-recorder residual note
