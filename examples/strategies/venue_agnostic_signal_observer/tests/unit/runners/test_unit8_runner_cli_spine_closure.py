...
class TestUnit8RunnerCliSpineClosure:
    ...
    def test_oi_velocity_package_root_import_remains_metadata_light(self) -> None:
        code = textwrap.dedent(
            f"""
            import sys
            before = set(sys.modules)
            ...
            after = set(sys.modules)
            new = after - before
            hits = [name for name in new if name == {_OI_RUNNER_MODULE!r}]
            print('hits', hits)
            print('has_metadata', hasattr(package, 'METADATA'))
    ...
    def test_oi_velocity_legacy_wrapper_still_exposes_compat_symbols_when_explicitly_imported(self) -> None:
        code = textwrap.dedent(
            f"""
    ...
    def test_catalog_and_registry_imports_do_not_pull_old_run_py_modules(self) -> None:
        code = textwrap.dedent(
            ...
            after = set(sys.modules)
            new = after - before
            forbidden = [{_COST_RUN_MODULE!r}, {_OI_RUN_MODULE!r}, {_REGISTRY_CLI_MODULE!r}]
            hits = [name for name in new if name in forbidden]
            print('hits', hits)
            raise SystemExit(1 if hits else 0)
            """
