...
Verifies the 39 safely-migrated run_*.py files now live under
...
def test_root_run_files_are_all_safety_blocked() -> None:
    ...
    assert len(retained) == len(root) == 27, (len(retained), len(root))
...
def test_registry_and_spec_backed_clis_remain_at_root() -> None:
    # The registry/CLI-spec-backed flagship CLIs are metadata-pinned and stay at root.
    for key in (
        ...
        assert not spec.path.startswith(_LEGACY_CLI_PREFIX), key
