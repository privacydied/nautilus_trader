...
class TestCliSpecsCatalog:
    ...
    def test_catalog_contains_exactly_one_spec_in_unit8c(self) -> None:
        assert len(ALL_RUNNER_CLI_SPECS) == 1

    def test_the_only_key_is_hyperliquid_cost_feasibility(self) -> None:
    ...
    def test_catalog_order_is_deterministic(self) -> None:
        assert ALL_RUNNER_CLI_SPECS == (HYPERLIQUID_COST_FEASIBILITY_CLI_SPEC,)

    def test_importing_cli_specs_does_not_import_cost_feasibility_runner(self) -> None:
        code = textwrap.dedent(
            f"""
            import sys
            before = set(sys.modules)
    ...
    def test_importing_cli_specs_does_not_import_old_run_hyperliquid_cost_feasibility(self) -> None:
        code = textwrap.dedent(
            f"""
            import sys
            before = set(sys.modules)
    ...
    def test_importing_cli_specs_does_not_import_node_fills_runner(self) -> None:
        code = textwrap.dedent(
            """
            import sys
            before = set(sys.modules)
    ...
    def test_registry_output_remains_unchanged_and_does_not_consume_cli_specs(self) -> None:
        code = textwrap.dedent(
            """
            import json
            import sys
            ...
            cli_specs_module = (
                "examples.strategies.venue_agnostic_signal_observer.runners.cli_specs"
            )
            sys.modules.pop(cli_specs_module, None)
            sys.modules.pop(
            ...
            rc = main(["--list", "--json"])
            assert rc == 0, rc
            assert cli_specs_module not in sys.modules
            print("OK")
            """
        )
        result = subprocess.run(
