...
PACKAGE = "examples.strategies.venue_agnostic_signal_observer.hypotheses.legacy.generic_altcoin_stress_regime_ablation_phase0b"
LEGACY = "examples.strategies.venue_agnostic_signal_observer.generic_altcoin_stress_regime_ablation_phase0b"
CLI = "examples.strategies.venue_agnostic_signal_observer.runners.legacy_cli.run_generic_altcoin_stress_regime_ablation_phase0b"

EXPECTED_HELP_LINES = 15
HELP_KEY_SUBSTRINGS = (
    "--phase0a-report",
    "--archive-path",
    "--precommitment",
...
def _root_wrapper_path() -> Path:
    return _repo_root() / "examples/strategies/venue_agnostic_signal_observer/generic_altcoin_stress_regime_ablation_phase0b.py"


def _package_dir() -> Path:
    return _repo_root() / "examples/strategies/venue_agnostic_signal_observer/hypotheses/legacy/generic_altcoin_stress_regime_ablation_phase0b"
...
assert metadata.METADATA.key == "generic_altcoin_stress_regime_ablation_phase0b"
hits = [name for name in new if name.endswith(".generic_altcoin_stress_regime_ablation_phase0b.runner")]
print("\\n".join(hits))
...
after = set(sys.modules)
new = sorted(after - before)
assert pkg.METADATA.key == "generic_altcoin_stress_regime_ablation_phase0b"
hits = [name for name in new if name.endswith(".generic_altcoin_stress_regime_ablation_phase0b.runner")]
print("\\n".join(hits))
raise SystemExit(1 if hits else 0)
'''
...
def test_runner_imports_directly() -> None:
    ...
    runner = importlib.import_module(f"{PACKAGE}.runner")
    assert callable(runner.run_phase0b)
